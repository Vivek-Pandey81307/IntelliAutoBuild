  ...
  try {
    ...
    if (!!data?.ok) {
      ...
      return NextResponse.redirect(
        `https://intelliautobuild.vercel.app/connections?app_id=${appId}&authed_user_id=${userId}&authed_user_token=${userToken}&slack_access_token=${accessToken}&bot_user_id=${botUserId}&team_id=${teamId}&team_name=${teamName}`
      )
    }
  } catch (error) {
    console.error(error)
